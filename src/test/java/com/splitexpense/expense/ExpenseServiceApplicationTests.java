...
/**
 * Boots the whole application against a real PostgreSQL and checks the three things the rest
 * of the service is built on top of.
 *
 ...
 *   <li>resilience4j 2.3.0 is the {@code spring-boot3} artifact running on Spring Boot 4. Its
 *       aspects were expected to survive that; asserting the registries exist and are
...
class ExpenseServiceApplicationTests {
    ...
    static PostgreSQLContainer postgres = new PostgreSQLContainer("postgres:16-alpine")
            .withDatabaseName("splitexpense_expense")
    ...
    @Test
    @DisplayName("Flyway applies V1 and creates all five tables")
    void migrationCreatesTheSchema() {
        ...
        assertThat(tables).contains(
                "expenses", "expense_splits", "settlements", "idempotency_records", "audit_logs");
    }

    /**
     * The idempotency design rests entirely on this constraint existing. If a future
     * migration drops or renames it, every duplicate-detection path in the service degrades
     * from "the database refuses the second insert" to "both inserts succeed and the expense
     * is recorded twice" — with no other test necessarily failing.
     */
    @Test
    @DisplayName("expenses.idempotency_key carries the UNIQUE constraint the saga relies on")
    ...
    @Test
    @DisplayName("The groupService breaker and retry bind the values declared in YAML")
    void resilienceConfigurationIsApplied() {
        CircuitBreakerConfig breaker =
                circuitBreakerRegistry.circuitBreaker("groupService").getCircuitBreakerConfig();
        ...
        assertThat(retryRegistry.retry("groupService").getRetryConfig().getMaxAttempts())
                .isEqualTo(3);
    }
}
