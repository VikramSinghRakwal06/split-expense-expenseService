    ...
    public OpenAPI paymentServiceOpenApi() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("v1")
                        .description("""
                                Idempotent transfers between wallets held by wallet-service. \
                                Every transfer is keyed by a client-supplied Idempotency-Key, \
                                so a retried request is always safe. Authenticates with \
                                access tokens issued by auth-service; this service verifies \
                                them and never mints one.""")
